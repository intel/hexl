  ...
  for (size_t r = 0; r < num_weights; r++) {
    ...
    for (size_t i = 0; i < num_moduli; i++) {
      ...
      intel::hexl::EltwiseMultMod(
          cipher2 + poly2_offset, cipher0 + poly1_offset,
          cipher1 + poly1_offset, poly_size, moduli[i], 1);

      // Compute second output polynomial
      // result[1] = x[1] * y[0]
      intel::hexl::EltwiseMultMod(
          cipher2 + poly1_offset, cipher0 + poly1_offset,
          cipher1 + poly0_offset, poly_size, moduli[i], 1);

      // result[1] = x[0] * y[1]
      intel::hexl::EltwiseMultMod(temp.data(), cipher0 + poly0_offset,
                                  cipher1 + poly1_offset, poly_size, moduli[i],
                                  1);
      // result[1] += temp_poly
      intel::hexl::EltwiseAddMod(cipher2 + poly1_offset, cipher2 + poly1_offset,
                                 temp.data(), poly_size, moduli[i]);

      // Compute first output polynomial
      // result[0] = x[0] * y[0]
      intel::hexl::EltwiseMultMod(
          cipher2 + poly0_offset, cipher0 + poly0_offset,
          cipher1 + poly0_offset, poly_size, moduli[i], 1);
    }
  }
    ...
    for (size_t s = 0; s < num_weights; s += step) {
      ...
      for (size_t i = 0; i < num_moduli; i++) {
        ...
        intel::hexl::EltwiseAddMod(
            left_cipher + poly0_offset, right_cipher + poly0_offset,
            left_cipher + poly0_offset, poly_size, moduli[i]);

        intel::hexl::EltwiseAddMod(
            left_cipher + poly1_offset, right_cipher + poly1_offset,
            left_cipher + poly1_offset, poly_size, moduli[i]);

        intel::hexl::EltwiseAddMod(
            left_cipher + poly2_offset, right_cipher + poly2_offset,
            left_cipher + poly2_offset, poly_size, moduli[i]);
      }
    }
  }
#else
