...
/// modulus)
// NOTE: this function is not a cryptographically secure random number generator
inline uint64_t GenerateUniformRandomValue(uint64_t modulus) {
...
inline AlignedVector64<uint64_t> GenerateUniformRandomValues(uint64_t size,
  ...
  auto generator = [&modulus]() { return GenerateUniformRandomValue(modulus); };
