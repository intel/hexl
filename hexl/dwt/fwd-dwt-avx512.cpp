...
/// @brief Final butterfly step for the Forward DWT.
...
void ComplexFwdDWT_Butterfly(__m512d* X_real, __m512d* X_imag, __m512d* Y_real,
...
// imaginary part.
void ComplexFwdDWT_T1(double* operand_8C_intrlvd, const double* W_1C_intrlvd,
  ...
  for (size_t i = 0; i < (m >> 1); i += 8) {
    ...
    ComplexFwdDWT_Butterfly(&v_X_real, &v_X_imag, &v_Y_real, &v_Y_imag,
                            v_W_real, v_W_imag);
...
void ComplexFwdDWT_T2(double* operand_8C_intrlvd, const double* W_1C_intrlvd,
                      uint64_t m) {
  size_t offset = 0;
...
void ComplexFwdDWT_T4(double* operand_8C_intrlvd, const double* W_1C_intrlvd,
                      uint64_t m) {
  size_t offset = 0;
...
void ComplexFwdDWT_T8(double* operand_8C_intrlvd, const double* W_1C_intrlvd,
...
void ComplexStartFwdDWT_T8(double* result_8C_intrlvd,
                           const double* operand_1C_intrlvd,
                           const double* W_1C_intrlvd, uint64_t gap,
                           uint64_t m) {
  size_t offset = 0;
...
void Forward_DWT_ToBitReverseAVX512(
  ...
  if (n <= base_dwt_size) {  // Perform breadth-first DWT
    ...
    if (recursion_depth == 0 && gap >= 16) {
      ...
      ComplexStartFwdDWT_T8(result_cmplx_intrlvd, operand_cmplx_intrlvd,
    ...
    for (; gap >= 16; gap >>= 1) {
      const double* W_cmplx_intrlvd =
          &root_of_unity_powers_cmplx_intrlvd[W_idx];
      ComplexFwdDWT_T8(result_cmplx_intrlvd, W_cmplx_intrlvd, gap, m);
      m <<= 1;
      W_idx <<= 1;
    }

    {
      ...
      const double* W_cmplx_intrlvd =
          &root_of_unity_powers_cmplx_intrlvd[W_idx];
      ComplexFwdDWT_T4(result_cmplx_intrlvd, W_cmplx_intrlvd, m);
      m <<= 1;
      W_idx <<= 1;

      // T2
      W_cmplx_intrlvd = &root_of_unity_powers_cmplx_intrlvd[W_idx];
      ComplexFwdDWT_T2(result_cmplx_intrlvd, W_cmplx_intrlvd, m);
      m <<= 1;
      W_idx <<= 1;

      // T1
      W_cmplx_intrlvd = &root_of_unity_powers_cmplx_intrlvd[W_idx];
      ComplexFwdDWT_T1(result_cmplx_intrlvd, W_cmplx_intrlvd, m, scale);
    }
  } else {
    // Perform depth-first DWT via recursive call
    size_t gap = n;
    ...
    if (recursion_depth == 0) {
      ...
      ComplexFwdDWT_T8(result_cmplx_intrlvd, W_cmplx_intrlvd, gap, 2);
    }

    Forward_DWT_ToBitReverseAVX512(result_cmplx_intrlvd, result_cmplx_intrlvd,
  ...
  if (recursion_depth == 0) {
    HEXL_VLOG(
        5,
        "AVX512 returning FWD DWT result " << std::vector<std::complex<double>>(
