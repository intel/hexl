...
/// @brief Performs linear forward and inverse DWT-like transform
...
class DWT {
 public:
  ...
  /// @brief Initializes an DWT object with degree \p degree and scalar
  /// \p in_scalar.
  /// @param[in] degree also known as N. Size of the DWT transform. Must be a
  /// power of 2
  ...
  /// transforms
  DWT(uint64_t degree, double* in_scalar,
      std::shared_ptr<AllocatorBase> alloc_ptr = {});

  template <class Allocator, class... AllocatorArgs>
  DWT(uint64_t degree, double* in_scalar, Allocator&& a,
      AllocatorArgs&&... args)
      : DWT(degree, in_scalar,
  ...
  /// @brief Compute forward DWT. Results are bit-reversed.
  /// @param[out] result Stores the result
  /// @param[in] operand Data on which to compute the DWT
  /// @param[in] in_scale Scale applied to output values
  void ComputeForwardDWT(std::complex<double>* result,
                         const std::complex<double>* operand,
                         const double* in_scale = nullptr);

  /// @brief Compute inverse DWT. Results are bit-reversed.
  /// @param[out] result Stores the result
  /// @param[in] operand Data on which to compute the DWT
  /// @param[in] in_scale Scale applied to output values
  void ComputeInverseDWT(std::complex<double>* result,
                         const std::complex<double>* operand,
                         const double* in_scale = nullptr);
 ...
 private:
  ...
  uint64_t m_degree;  // N: size of DWT transform, should be power of 2

  double* scalar;  // Pointer to scalar used for scale/inv_scale calculation

  double scale;  // Scale value use for encoding (inv dwt)

  double inv_scale;  // Scale value use in decoding (fwd dwt)
