...
DWT::DWT(uint64_t degree, double* in_scalar,
...
void DWT::ComputeComplexRootsOfUnity() {
...
void DWT::ComputeForwardDWT(std::complex<double>* result,
  ...
  HEXL_VLOG(3, "Calling 64-bit AVX512-DQ FwdDWT");
  ...
  return;
#else
  HEXL_VLOG(3, "Calling Native FwdDWT");
  Forward_DWT_ToBitReverseRadix2(
      result, operand, m_complex_roots_of_unity.data(), m_degree, out_scale);
  return;
#endif
}

void DWT::ComputeInverseDWT(std::complex<double>* result,
                            const std::complex<double>* operand,
                            const double* in_scale) {
  HEXL_CHECK(result != nullptr, "result==nullptr");
  HEXL_CHECK(operand != nullptr, "operand==nullptr");
...
#ifdef HEXL_HAS_AVX512DQ
  HEXL_VLOG(3, "Calling 64-bit AVX512-DQ InvDWT");
  ...
  return;
#else
  HEXL_VLOG(3, "Calling Native InvDWT");
  Inverse_DWT_FromBitReverseRadix2(result, operand,
...
void DWT::BuildFloatingPoints(std::complex<double>* res, const uint64_t* plain,
