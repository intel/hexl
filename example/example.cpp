...
void ExampleEltwiseAddMod() {
  std::cout << "Running ExampleEltwiseAddMod...\n";

  std::vector<uint64_t> op1{1, 2, 3, 4, 5, 6, 7, 8};
  std::vector<uint64_t> op2{1, 3, 5, 7, 2, 4, 6, 8};
  uint64_t modulus = 10;
  std::vector<uint64_t> exp_out{2, 5, 8, 1, 7, 1, 3, 6};

  intel::hexl::EltwiseAddMod(op1.data(), op1.data(), op2.data(), modulus,
                             op1.size());

  CheckEqual(op1, exp_out);
  std::cout << "Done running ExampleEltwiseAddMod\n";
}
